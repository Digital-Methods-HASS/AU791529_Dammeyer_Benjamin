---
title: "Danish Kings"
author: "Benjamin Dammeyer and Anders Pedersen"
date: "2026-13-03"
...
The two options with _ is a tidyverse function, and . is an R function. We are using the kings4 <- read_csv2("data/danish_kings.csv") as our data is seperated with ; hence the csv2.
...
When we use ncol on kings 1-4 we see that kings 1-2 shows us 1 culumn and kings 3-4 shows us 6 columns. This is because our data is seperated by ; and not , hence the usage of csv2
...
```{r}
#tryings the base-r function
mean(kings, na.rm = TRUE)

#trying the summarize
kings %>%
...
```{r}
#creating column and preparing the DATA
kings <- kings %>%
...
```{r}
#telling R that we are using the dataset kings
kings %>%
  ggplot()
```
...
```{r}
#defining which axel midyear and duration goes on.
kings %>%
...
```{r}
#adds a line following the scatterplot
...
kings %>% 
  ...
  geom_point()+
  geom_smooth()+
  labs(title="duration_of_kings_in_Denmark", x="midyear of reign", y="duration of reign (kings)")+
  theme_bw() +
