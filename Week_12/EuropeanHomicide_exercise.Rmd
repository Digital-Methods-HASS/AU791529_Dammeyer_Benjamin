...
names(Western_Europe)[4] <- "Homicides_per_100.000"
...
Western_Europe %>% 
  ggplot(aes(x = Year, y = Homicides_per_100.000, color=Entity))+
  geom_line()+
  theme_minimal()+
...
Western_Europe %>% 
  ggplot(aes(x = Year, y = `Homicides_per_100.000`, color = Countries)) +
  geom_line() +
  theme_minimal() +
  theme(
...
kings %>% 
  mutate(Duration = reign_end - reign_start,midyear = reign_end - (reign_end - reign_start)/2) %>% 
  ggplot(aes(x=midyear,y=Duration))+
  geom_point(na.rm = TRUE)+
  labs(title = "Duration of Danish Monarchs' Reigns Over Time",
...
#Based on the data visualizations, the answer is yes—at least when it comes to violence. The homicide rate graphs show a clear pattern across Western Europe. Looking at countries like England, Italy, Germany, France, and the Netherlands, every single one shows the same trend: homicides have declined significantly over the centuries. Take England as an example. In 1300, the homicide rate was around 23 murders per 100,000 people. By 1550 it had dropped to 7. By 1725 it was down to 2. Today it's near zero. This represents a fundamental shift in how people live together. What's notable is that this happened across all of Western Europe, not just in one country. Different cultures, different political systems—but all moved in the same direction: toward less violence. The Danish kings plot shows something different. Reign durations varied considerably depending on when rulers died. But that's about political transitions at the top. The homicide data tells us about ordinary people and everyday life. Something changed over these centuries. Rates of lethal violence dropped steadily. People found other ways to resolve conflicts. Governments became more effective at maintaining order. Life became safer. By this measure—how likely you were to be murdered—we are indeed more civilized than our ancestors. The data shows a consistent, long-term decline that's hard to argue with.
